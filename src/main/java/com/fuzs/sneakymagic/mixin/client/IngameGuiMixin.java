        ...
        if (element.isEnabled()) {
            ...
            if (this.highlightingItemStack.isEnchanted() || element.affectBooks && this.highlightingItemStack.getItem() == Items.ENCHANTED_BOOK) {
                ...
                if (element.colorName && CurseMatcher.anyMatch(enchantments)) {
                    ...
                    return component.applyTextStyle(handleEnchantedBook ? ((IItemAccessor) this.highlightingItemStack.getItem()).getRarity().color : TextFormatting.RED);
                }
            }
        }

        return component.applyTextStyle(format);
    }
